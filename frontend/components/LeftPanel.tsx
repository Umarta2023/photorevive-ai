...
const LeftPanel: React.FC<LeftPanelProps> = ({
  ...
  const getButtonText = () => {
    ...
    return `${t.processImageBtn} (${processCost} ${t.creditsLabel})`;
  };
  ...
  return (
    <div className="w-1/4 max-w-sm flex flex-col bg-gray-800 p-6 border-r border-gray-700 space-y-6 flex-shrink-0">
      <header className="flex items-start justify-between">
        <div>
            <h1 className={`font-bold text-yellow-400 whitespace-nowrap ${language === 'en' ? 'text-xl' : 'text-lg'} flex items-center`}>
                <LogoIcon 
                    className="w-8 h-8 inline-block align-middle mr-2 text-yellow-400"
                />
                {t.appTitle}
            </h1>
        ...
        <div className="flex items-center space-x-2">
          <div className="flex items-center space-x-2 bg-gray-700/50 px-3 py-1.5 rounded-full text-sm font-semibold">
              <CreditIcon className="w-5 h-5 text-yellow-400"/>
              <span className="text-gray-100">{userCredits}</span>
              <button onClick={onAddCredits} className="ml-1 text-gray-400 hover:text-yellow-300 transition-colors" title={t.addCredits}>
      ...
      <div className="flex-grow flex flex-col space-y-6 overflow-y-auto pr-2 -mr-2">
        <ImageUploader onImageUpload={onImageUpload} currentImage={processingImageUrl} />
