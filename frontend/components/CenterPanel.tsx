...
const CenterPanel: React.FC<CenterPanelProps> = ({
  ...
  const isComparisonDisabled = selectedResult?.prompt === 'Image Edited';
  ...
  const renderContent = () => {
    ...
    if (error) {
      const isCreditError = error === t.notEnoughCreditsError;
      return (
        <div className={`flex flex-col items-center justify-center h-full text-red-400 bg-red-900/20 p-8 rounded-lg ${isCreditError ? 'w-full max-w-md' : ''}`}>
          <p className="font-bold text-lg mb-2">{isCreditError ? error : t.errorTitle}</p>
          <p className="text-center">{isCreditError ? t.notEnoughCreditsBody : error}</p>
          {isCreditError && (
              <button 
                onClick={onOpenPaymentModal}
                className="mt-6 w-full flex items-center justify-center bg-yellow-400 text-gray-900 font-bold py-2 px-4 rounded-lg shadow-md hover:bg-yellow-300 transition-all"
              >
                  <CreditIcon className="h-5 w-5 mr-2"/>
                  {t.topUpBtn}
              </button>
          )}
        </div>
